...
class ThingsConsume extends Thing {
    constructor(td:any) {
        ...
        let metaData: any = {}
        // Add protocol Dependent keyboards
        for (const protocol in td.protocolList) {
            metaData = {}
            // adding the property
            Object.entries(td.protocolList[protocol]).forEach(([key, value]) => {
                metaData[key] = value
            })
            this.addProtocol(
                new Protocol(protocol, metaData)
            );
        }
